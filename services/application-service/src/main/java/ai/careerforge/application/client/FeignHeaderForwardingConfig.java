 ...
 *
 * <p>application-service calls jd-service, resume-service and assessment-service directly
 * (Eureka, not through the gateway), so those services never see the gateway's own
 * header-stripping/re-setting step. The header is trustworthy here for the same reason it
 * is trustworthy at the gateway: these services are reachable only on the internal network
 * (docs/CODEBASE.md &sect;4).
 */
