...
/**
 * Connects the existing JD-optimization flow (jd-service, ADR-033) to render-service's rendering
 * pipeline (ADR-036): JD → JD optimization data → render-service → PDF.
 *
 ...
 * Content operation and a full {@code ResumeDocumentModel} assembly step remain a separate,
 * larger piece of ADR-036 this integration does not implement.
 *
 * <p>Template selection is untouched: {@link Application#templateId()} continues to mean
 * exactly what it already did (validated at application-creation time, unrelated to this flow).
 * render-service has exactly one built-in layout today ({@code RenderTemplate.STANDARD}), so
 * that is what every render request asks for, regardless of {@code templateId} — a second
 * built-in layout arriving later is a one-line change here, not a redesign.
 *
...
public class ResumeRenderService {
    ...
    /** Evidence types in resume section order — the same six codes
     *  {@code ai-service}'s {@code EvidenceItem} always uses, mapped 1:1 to render-service's
     *  ATS-standard {@code SectionHeading} values. */
